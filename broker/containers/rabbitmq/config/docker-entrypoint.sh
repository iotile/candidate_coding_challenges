...
if [ "$1" = 'rabbitmq-server' ] && [ "$shouldWriteConfig" ]; then
	...
	if [ "$(rabbitmq-plugins list -q -m -e rabbitmq_auth_backend_http)" ]; then
		auth_http_server=machineapi:5000
		rabbit_set_config "auth_backends.1" internal
		rabbit_set_config "auth_backends.2" cache
		rabbit_set_config "auth_cache.cached_backend" http
		rabbit_set_config "auth_cache.cache_ttl" 300000  # 5 min
		rabbit_set_config "auth_backends.3" http
		rabbit_set_config "auth_http.http_method" post
		rabbit_set_config "auth_http.user_path" http://$auth_http_server/machine/api/v1/rabbitmq-auth/user
		rabbit_set_config "auth_http.vhost_path" http://$auth_http_server/machine/api/v1/rabbitmq-auth/vhost
		rabbit_set_config "auth_http.resource_path" http://$auth_http_server/machine/api/v1/rabbitmq-auth/resource
		rabbit_set_config "auth_http.topic_path" http://$auth_http_server/machine/api/v1/rabbitmq-auth/topic
		fi
	...
	if [ "$(rabbitmq-plugins list -q -m -e rabbitmq_management)" ]; then
		...
		if [ -f "$managementDefinitionsFile" ]; then
			...
			# ARCH ADDED: Patch in the user and hashed password to the management file
			HASHED_PASS=$(python /usr/local/bin/generate_password.py $BROKER_PASS)
			jq -M ".permissions[0].user=\"$BROKER_USER\" | .users[0].name=\"$BROKER_USER\" | .users[0].password_hash=\"$HASHED_PASS\"" /etc/rabbitmq/definitions.json > /etc/rabbitmq/definitions.json.new
			mv -f /etc/rabbitmq/definitions.json.new /etc/rabbitmq/definitions.json

			HASHED_RW_PASS=$(python /usr/local/bin/generate_password.py $BROKER_RW_PASS)
			jq -M ".permissions[1].user=\"$BROKER_RW_USER\" | .users[1].name=\"$BROKER_RW_USER\" | .users[1].password_hash=\"$HASHED_RW_PASS\"" /etc/rabbitmq/definitions.json > /etc/rabbitmq/definitions.json.new
			mv -f /etc/rabbitmq/definitions.json.new /etc/rabbitmq/definitions.json

			HASHED_RO_PASS=$(python /usr/local/bin/generate_password.py $BROKER_RO_PASS)
			jq -M ".permissions[2].user=\"$BROKER_RO_USER\" | .users[2].name=\"$BROKER_RO_USER\" | .users[2].password_hash=\"$HASHED_RO_PASS\"" /etc/rabbitmq/definitions.json > /etc/rabbitmq/definitions.json.new
			mv -f /etc/rabbitmq/definitions.json.new /etc/rabbitmq/definitions.json
